...
@export var wander_range : int = 2 : set = _set_wonder_rang
...
func start() -> void:
	...
	npc.update_animation()
	await get_tree().create_timer(randf() * wander_duration + idle_duration * 0.5).timeout
...
func _set_wonder_rang(v: int) -> void:
	wander_range = v
